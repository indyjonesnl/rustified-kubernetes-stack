...
NODE="${KIND_NODE:-crun-control-plane}"
NS=smoke
...
echo "==> Verifying containers ran on crun (the OCI runtime)"
# crun keeps container state under containerd's runc state root, so if crun itself
# lists running containers, crun is genuinely executing the cluster's pods (driven
# by the stock Go runc-v2 shim via BinaryName -> crun) -- not just configured.
CRUN_RUNNING=$(docker exec "$NODE" crun --root /run/containerd/runc/k8s.io list 2>/dev/null | grep -ciE ' running ' || true)
test "$CRUN_RUNNING" -gt 0 || { echo "FAIL: crun reports no running containers"; exit 1; }
echo "crun running containers: $CRUN_RUNNING"
# sanity: containerd's default runtime really points its OCI binary at crun
docker exec "$NODE" crictl info 2>/dev/null | grep -q '"BinaryName": "/usr/local/bin/crun"' \
  || { echo "FAIL: containerd default runtime BinaryName is not crun"; exit 1; }

echo "PASS: kind-containerd-crun-coredns smoke test"
